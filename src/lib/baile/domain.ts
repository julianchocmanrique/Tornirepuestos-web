...
export function enroll(s: State, user: User, categoryId: string, team: string, members: string) {
  ...
  ensure(s.settings.categoryRegistrationOpen, "Las inscripciones a categorías están cerradas.");
  ...
  ensure(!s.schedule.some(x => x.categoryId === c.id && ["live", "finished"].includes(x.status)), "Esta categoría ya comenzó su competencia.");
...
export function review(s: State, admin: User, paymentId: string, status: Status, note: string) {
  ...
  if (p.kind === "fullpass" && status !== "approved") ensure(!s.accreditations.some(a => a.participantId === p.participantId && !a.revokedAt), "Primero anula la acreditación de este participante.", 409);
  if (p.enrollmentId && status === "approved") {
    const entry = s.enrollments.find(e => e.id === p.enrollmentId)!;
    ensure(!s.schedule.some(x => x.categoryId === entry.categoryId && ["live", "finished"].includes(x.status)), "La categoría ya comenzó. No se pueden habilitar nuevas inscripciones.", 409);
  }
  const affected = s.enrollments.filter(e => p.enrollmentId ? e.id === p.enrollmentId : e.participantId === p.participantId);
  ensure(!s.schedule.some(x => ["live", "finished"].includes(x.status) && affected.some(e => e.categoryId === x.categoryId)), "No se pueden cambiar pagos de una competencia iniciada o finalizada.", 409);
...
export function saveScore(s: State, judge: User, enrollmentId: string, values: number[]) {
  ...
  ensure(!s.schedule.some(x => x.categoryId === e.categoryId && x.status === "finished"), "La categoría ya finalizó y sus resultados están cerrados.", 409);
...
export function scope(s: State, user: User | null): Snapshot {
  ...
  const base: Snapshot = { settings: s.settings, user: publicUser, participants: [], payments: [], categories: s.categories.filter(c => c.active), enrollments: [], scores: [], judges: [], audit: [], accreditations: [], notes: [], schedule: [], exampleCreated: s.audit.some(x => x.action === "Ejemplo creado") };
  if (!user) return base;
  if (user.role === "admin") return { ...base, categories: s.categories, participants: s.participants, payments: s.payments, enrollments: s.enrollments, scores: s.scores, audit: s.audit.slice(-150).reverse(), accreditations: s.accreditations, notes: s.notes, schedule: s.schedule, judges: s.users.filter(u => u.role === "judge").map(({ passwordHash: _ignored, ...u }) => u) };
  if (user.role === "participant") return { ...base, categories: s.categories.filter(c => c.active || s.enrollments.some(e => e.categoryId === c.id && e.participantId === user.id)), participants: s.participants.filter(p => p.id === user.id), payments: s.payments.filter(p => p.participantId === user.id), enrollments: s.enrollments.filter(e => e.participantId === user.id), accreditations: s.accreditations.filter(a => a.participantId === user.id && !a.revokedAt), schedule: s.schedule.filter(x => x.status !== "cancelled" && s.enrollments.some(e => e.categoryId === x.categoryId && e.participantId === user.id)).map(x => ({ ...x, note: "" })) };
...
export function recordPayment(s: State, user: User, enrollmentId: string | null, amount: number, date: string, receiptId: string): Payment {
  ...
  ensure(!e || !s.schedule.some(x => x.categoryId === e.categoryId && ["live", "finished"].includes(x.status)), "La competencia ya comenzó y no admite nuevos comprobantes.", 409);
  ...
  ensure(amount >= (e ? e.fee : s.participants.find(p => p.id === user.id)?.fullPassFee ?? s.settings.fullPassFee), "El valor reportado es menor al valor de inscripción.");
